...
Market Microstructure Project 1 Market Maker - All Days 
...
def Plot_Price_Volume (Date,Volume,Price,df) : 
    ...
    color = 'blue'
    ax1.set_xlabel('Date')
    ax1.set_ylabel('EFFECTIVE SPREAD AVG', color=color)
    ax1.plot(x, y, label=Volume)
    ...
    color = 'red'
    ax2.set_ylabel('OFI', color=color)  # we already handled the x-label with ax1
...
Plot_Price_Volume ('DATE_df_trades', 'EFFECTIVE SPREAD AVG', 'OFI', df_EF_OFI)
